...
#ifndef __WAVESMVVTP_H
#define __WAVESMVVTP_H
...
template<class TYPE>
MV_Vector<TYPE>::MV_Vector(unsigned int n) :
...
template<class TYPE>
MV_Vector<TYPE>::MV_Vector(const MV_Vector<TYPE> & m) :
		p_(new TYPE[m.dim_]), dim_(m.dim_), ref_(0)
{
	if (p_ == NULL)
	{
...
template<class TYPE>
MV_Vector<TYPE>::MV_Vector(TYPE* d, unsigned int n) :
		p_(new TYPE[n]), dim_(n), ref_(0)
{
	if (p_ == NULL)
	{
...
template<class TYPE>
MV_Vector<TYPE>::MV_Vector(const TYPE* d, unsigned int n) :
		p_(new TYPE[n]), dim_(n), ref_(0)
{
	if (p_ == NULL)
	{
...
MV_Vector<TYPE> MV_Vector<TYPE>::operator()(const MV_VecIndex &I)
{
	...
	else
	{
		...
		if (I.end() >= dim_)
		{
			cerr << "MV_VecIndex: (" << I.start() << ":" << I.end() << ") too big for matrix (0:" << dim_ - 1 << ") " << endl;
			exit(1);
		}
